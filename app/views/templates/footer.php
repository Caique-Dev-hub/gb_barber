<footer>
    <div class="site">
        <div class="footer">
            <div class="box">
                <h3>Links Rápidos</h3>
                <ul style="width: 380px; height: 200px;text-align: center;">
                    <li style=><a  class="session-menu" data-destiny="header">Inicio</a></li>
                    <li  style=><a class="session-menu" data-destiny="servico" >Serviços</a></li>
                    <li style=><a  class="session-menu" data-destiny="sobre">Sobre</a></li>
                    <li style=""><a  class="session-menu"  data-destiny="contato" >Contato</a></li>
                </ul>
            </div>
            <div class="box">
                <h3>Contato</h3>
                <ul style="font-size: 1.8rem; text-align: center;" class="local">
                    <li>Rua Henrique Jacobs, 689 - Vila Santa Teresa São Paulo - SP</li>
                    <li>(11) 95853-8834</li>
                    <li>gabrielgb@gmail.com</li>
                </ul>
            </div>
            <div class="share">
                <h3>Nos Siga</h3>
                <div style="text-align: center;">
                    <img style=""
                        src="data:image/png;base64,"
                        alt="instagram-new">
                    <img style=""
                        src="data:image/png;base64,"
                        alt="facebook-new">
                    <img style=""
                        src="data:image/png;base64,"
                        alt="twitterx--v2">
                </div>
            </div >
        </div>
        <div class="creditos">
            <div>
                <ul>
                    <li>@2025 GB-BARBEARIA. Todos os direitos reservados</li>
                    <li>Desenvolvido por Codex-lab</li>
                </ul>
            </div>

        </div>
    </div>
</footer>

<script>
    document.addEventListener('DOMContentLoaded', function(){
        document.querySelectorAll('.session-menu').forEach((header) => {
            header.addEventListener('click', function(){
                let destino = this.dataset.destiny;

                document.getElementById(`${destino}`).scrollIntoView({behavior: "smooth"});
            })
        })
    })
</script>